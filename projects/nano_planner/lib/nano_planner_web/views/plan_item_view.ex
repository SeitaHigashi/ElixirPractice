defmodule NanoPlannerWeb.PlanItemView do
  ...
  defp format_starts_at(item) do
    ...
    if item.starts_at.year == Timex.now(time_zone).year do
      Strftime.format!(item.starts_at, "%-m 月 %-d 日 (#{w}) %H:%M")
    else
      Strftime.format!(item.starts_at, "%Y 年 %-m 月 %-d 日 (#{w}) %H:%M")
    end
  end
  ...
  defp format_ends_at(item) do
    ...
    cond do
      ...
      item.ends_at.year == item.starts_at.year ->
        Strftime.format!(item.ends_at, "%-m 月 %-d 日 (#{w}) %H:%M")

      true ->
        Strftime.format!(item.ends_at, "%Y 年 %-m 月 %-d 日 (#{w}) %H:%M")
    end
  end

  def format_datetime(datetime) do
    w = format_wday(datetime)
    Strftime.format!(datetime, "%Y 年 %-m 月 %-d 日 (#{w}) %H:%M")
  end
